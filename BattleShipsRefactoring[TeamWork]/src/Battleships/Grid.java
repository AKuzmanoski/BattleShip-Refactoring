...
public class Grid implements Serializable {
	...
	public boolean checkMinePlaced() {
		if (!ships.containsKey("Minesweeper"))
			return false;
	...
	public boolean addMine(int i, int j, int s) {
		...
		try {
			if (!checkMinePlaced()) {
				ships.put("Minesweeper", new Minesweeper(this, i, j,
						isHorizontal));
			}
		}
	...
	public boolean checkSubPlaced() {
		if (!ships.containsKey("Submarine"))
			return false;
	...
	public boolean addAir(int i, int j, int s) {
		...
		try {
			if (!checkAirPlaced()) {
				ships.put("AircraftCarrier", new AircraftCarrier(this, i, j,
						isHorizontal));
			}
		}
	...
	public boolean checkDestPlaced() {
		if (!ships.containsKey("Destroyer"))
			return false;
	...
	public boolean checkBattlePlaced() {
		if (!ships.containsKey("Battleship"))
			return false;
	...
	public boolean addBattle(int i, int j, int s) {
		...
		try {
			if (!checkBattlePlaced()) {
				ships.put("Battleship",
						new Battleship(this, i, j, isHorizontal));
			}
		}
	...
	public boolean checkAirPlaced() {
		if (!ships.containsKey("AircraftCarrier"))
			return false;
	...
	public boolean shot(int i, int j) {
		...
		// ako ne e vo sekoj dr slucaj ke vrati false, samo vo sqr=0 ke napravi
		// plus update
		if (sqr == 0) {
			this.update(i, j, 1);
		}
	...
	/**
	 * Answers to the question whether the grid cell is empty (no ship is placed
	 * on).
	 * 
	 * @param i
	 *            the row index
	 * @param j
	 *            the column index
	 * @return <code>true</code> if cell is empty, <code>false</code> in every
	 *         other case
	 */
